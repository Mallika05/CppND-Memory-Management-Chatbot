...
ChatLogic::~ChatLogic()
{
    ...
    {
        delete *it;
    } delete is not necesarry as it's a unique pointer */ 

    // delete all edges
    //for (auto it = std::begin(_edges); it != std::end(_edges); ++it)
    //{
    //    delete *it;
    //} edges vector turned obsolete
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {
                            std::unique_ptr<GraphNode> newElement = std::make_unique<GraphNode>(id);
                            _nodes.emplace_back(std::move(newElement));
    ...
    ChatBot _chatbotLocal = ChatBot("../images/chatbot.png"); // create local chatbot instance
    SetChatbotHandle(&_chatbotLocal);
    _chatbotLocal.SetChatLogicHandle(this);
    // add chatbot to graph root node
    _chatbotLocal.SetRootNode(rootNode);
    // pass local chatbot instance to root node
